...
class JobCardSystem {
        ...
        this.customerSearchTimeout = setTimeout(async () => {
            try {
                const response = await fetch(
                    `https://fc3be690de421ba4-223-233-83-89.serveousercontent.com/api/customers/search?q=${encodeURIComponent(query)}`
                );
    ...
    async handleShipperSearch(event) {
        ...
        this.shipperSearchTimeout = setTimeout(async () => {
            try {
                const response = await fetch(
                    `https://fc3be690de421ba4-223-233-83-89.serveousercontent.com/api/shippers/search?q=${encodeURIComponent(query)}`
                );
